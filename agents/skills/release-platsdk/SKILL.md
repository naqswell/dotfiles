---
name: release-platsdk
description: Выпуск новой версии platsdk — релизная ветка, бамп, CHANGELOG, сборка, тег, публикация, back-merge в develop
---

Общие правила `~/.agents/AGENTS.md` обязательны: локальные коммиты разрешены, push выполняет только пользователь. Примеры с `git push` ниже — команды для показа пользователю, агент их не исполняет. Имена инструментов в исторических примерах заменяй доступными эквивалентами текущего клиента.
...
1. **Никогда не выполняй сам:** `git push`, `git tag -d`, любые `--force`,
   ...
   скажи «выполни её в своём терминале» и **остановись**. Не продолжай,
...
`аргументы запроса пользователя` разбирай так:
