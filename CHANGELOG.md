...
### Added
- Скиллы семейства `nqs` получили ключевые слова вместо угадывания синонимов, как уровни у `ponytail`: `report-qa` без аргумента шлёт сборку и текст за секунды, `report-qa срез` добавляет zip изменившихся кадров ценой прогона матрицы (2,5 мин на тёплом Gradle, до 10 на холодном); `send-gallery` по умолчанию отдаёт срез, `send-gallery полная` — все 2840 кадров; у `apk-send` в подсказке перечислены четыре формы вызова. Синоним нельзя проверить, и список синонимов растёт молча, а ключевое слово видно в `argument-hint` и в таблице режимов. Умолчание у `report-qa` перевёрнуто: раньше каждый QA-отчёт по platsdk ждал прогона матрицы, хотя тестировщику срез нужен не всегда
- Все пять скиллов семейства пишут свой текст по правилам `unslop`; `Skill` добавлен в `allowed-tools`, иначе тело звало бы инструмент, которого во фронтматтере нет. Формулировка именно «пиши по правилам», а не «прогони через»: `unslop` объявлен как «Must always apply» и в сессии обычно уже действует, так что команда вызвать его давала второй проход по уже чистому тексту — а он сам предупреждает, что вылизанный до стерильности текст заметен ровно так же, как канцелярит
- `report-qa` объясняет, почему сборка и отчёт уезжают двумя сообщениями: `tdl` шлёт по файлу на сообщение и альбомов не умеет (`-p` принимает каталоги, а альбом в Telegram — максимум 10 элементов), а через Bot API APK не пролезет: потолок 50 МБ и в «Избранное» бот писать не может. Отсюда строка «к сборке выше» в подписи отчёта — связать две половины посылки больше нечем

### Changed
- Отчёты стали плотнее. У `report-qa` было три раздела, из которых «что изменилось» и «как проверить» пересказывали одни и те же факты дважды, а формулировка «шагами, которые человек может повторить» приглашала расписывать нажатия. Теперь два раздела, одна строка на пункт и потолок в тридцать строк; у `report-design` — двадцать. Это не то, что чинит `unslop`: он режет канцелярит, а не избыточную полноту
