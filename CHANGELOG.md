...
- Все пять скиллов семейства прогоняют собственный текст через скилл `unslop` перед отправкой; `Skill` добавлен в `allowed-tools`, иначе тело звало бы инструмент, которого во фронтматтере нет
